...
public class GenericReader<T> {
    public Map<Integer, T> read(String fileName, Class<T> type, Json.Serializer<T> serializer, Function<T, Integer> function) {
    ...
    public Map<Integer, T> read(FileHandle file, Class<T> type, Json.Serializer<T> serializer, Function<T, Integer> function) {
        Json json = new Json();
